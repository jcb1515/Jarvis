/**
 * deep-link.ts — Handler for astronojarvis:// desktop deep links.
 *
 ...
 *   astronojarvis://research/{session_id}   → open a research session
 *   astronojarvis://connector/{connector_id} → open connector settings
 */
...
/**
 * Parse an `astronojarvis://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL (e.g. `astronojarvis://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `astronojarvis:` scheme.
 *
 * @example
 * parseDeepLink("astronojarvis://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("astronojarvis://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "astronojarvis:") return null;

    // The URL constructor treats "astronojarvis://research/abc123" such that
